...
public class Spline2DInspector : Editor {
	...
	private void ShowPoint (int index) {
		...
		if (Handles.Button(point, Quaternion.identity, size * handleSize, size * pickSize, Handles.RectangleHandleCap)) {
			selectedIndex = index;
			Repaint();
		}
		if (selectedIndex == index) {
			EditorGUI.BeginChangeCheck();
			var fmh_209_42_639011808292003960 = Quaternion.identity; point = Handles.FreeMoveHandle(point, size * 1.5f * handleSize, Vector3.zero, Handles.RectangleHandleCap);
			if (EditorGUI.EndChangeCheck()) {
				Undo.RecordObject(spline, "Move Point");
